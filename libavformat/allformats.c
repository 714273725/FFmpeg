...
static void av_format_init_next(void)
{
	...
	//muxer_list是一个静态不可改的数组，在编译FFMpeg时，根据configure生成的，在libavformat目录下的muxer_list.c中可以找到,其内容大概如下
	//static const AVOutputFormat * const muxer_list[] = {&ff_a64_muxer,&ff_ac3_muxer,&ff_adts_muxer...NULL}
	//里面的变量都是static const的
	//prevout 为空 则将prevout指向out
	//prevout 不为空 则将prevout的next 指向out,然后prevout再指向out
	//如此形成一个链表结构，每次循环后prevout都指向链表最后一个元素
	//eg:  第一次循环 out =A ：此时 prevout=NULL ,则 prevout = A  A->next=NULL  链表为A->NULL
	//     第二次循环 out =B ：此时 prevout = A  ,则 A->next = B  , prevout = B , B->next=NULL 链表为A->B->NULL
	//     第三次循环 out =C ：此时 prevout = B  ,则 B->next = C  , prevout = C , C->next=NULL 链表为A->B->C->NULL
	//略
